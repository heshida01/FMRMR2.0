...
class Dim_Rd(object):
    ...
    def __init__(self, file_csv, logger,args_i,args_s,args_e,args_l,args_m,args_t,args_c,args_o):
        self.file_csv = file_csv
        self.logger = logger

        self.args_i=args_i
        self.args_s = int(args_s)
        self.args_e = int(args_e)
        self.args_l = int(args_l)
        self.args_m = int(args_m)
        self.args_t = args_t
        self.args_c = args_c
        self.args_o = args_o
    ...
    def Result(self, seqmax, clf, features, csvfile):
        ...
        # f.write('f1 ={}\n '.format(f1))
        self.logger.info(('f1 ={:0.4f} '.format(f1)))
        acc = sklearn.metrics.accuracy_score(self.y, ypred, )
        self.logger.info('accuarcy = {:0.4f} '.format(acc))
        # f.write('accuarcy = {:} \n'.format(acc))
        precision = sklearn.metrics.precision_score(self.y, ypred, average='weighted')
        self.logger.info('precision ={:0.4f} '.format(precision))
        # f.write('precision ={} \n'.format(precision))
        recall = sklearn.metrics.recall_score(self.y, ypred, average='weighted')
        self.logger.info(('recall ={:0.4f}'.format(recall)))
        # f.write('recall ={}\n '.format(recall))
        ...
        auc = sklearn.metrics.roc_auc_score(y, ypred)
        # f.write('roc area = {}\n'.format(roc))
        self.logger.info('auc = {:0.4f}'.format(auc))
        ...
        if self.args_m == -1:
            ...
            columns_index = columns_index[0:self.args_m + 1]
            data = data[:, 0:self.args_m + 1]
    ...
    def Dim_reduction(self, features, features_sorted, outfile, csvfile):
        self.logger.info("Start dimension reduction ...")
        features_number = []
        ...
        with open(scorecsv, 'w') as f:
            ...
            for i in range(int(ceil((self.end - self.start) / self.length)) + 1):
                ...
                if self.args_t == "acc":
                    benchmark = acc
                elif self.args_t == "precision":
                    benchmark = precision
                elif self.args_t == "recall":
                    benchmark = recall
                elif self.args_t == "auc":
                    benchmark = auc
        ...
        self.logger.info('----------')
        self.logger.info('the max {} = {:0.4f}'.format(self.args_t, max))

        index_add1 = [x + 1 for x in seqmax]
        self.logger.info('{},length = {}'.format(self.df.columns.values[index_add1], len(seqmax)))
        self.logger.info('-----------')
        clf = RandomForestClassifier(random_state=1, n_estimators=100)
    ...
    def run(self, inputfile):
        ...
        outputfile = self.args_o
        #csvfile = self.args_c
        csvfile = os.getcwd() + os.sep + 'Results' + os.sep + os.path.basename(self.args_c)
        mrmr_featurLen = self.args_m
        features, features_sorted = feature_rank(file, self.logger, mrmr_featurLen)
        self.read_data()
        if int(self.args_e) == -1:
            self.args_e = len(pd.read_csv(file, engine='python').columns) - 1
        self.range_steplen(self.args_s, self.args_e, self.args_l)
...
def main(args_i,args_s=1,args_e=-1,args_l=1,args_m=-1,args_t='f1',args_c='',args_o=''):

    args_o = args_o.replace('Temp','Results')
    ...
    if int(args_e) == -1:
        args_e = len(pd.read_csv(file, engine='python').columns) - 1

    # global args_o
    # if args_o == '':
    #args_o = ''.join(args_i.split('.')[:-1]) + '.metrics.csv'


    d = Dim_Rd(file, logger,args_i,args_s,args_e,args_l,args_m,args_t,args_c,args_o)
    d.run(inputfile=file)
    outputfile = os.getcwd() + os.sep + 'Results' + os.sep + os.path.basename(args_o)
    csvfile = os.getcwd() + os.sep + 'Results' + os.sep + os.path.basename(args_c)
    ...
    pngpath = os.path.abspath('./Results') + os.sep + os.path.basename(args_i) + '.png'
    plt.savefig(pngpath)
    logger.info('Scatter charts visualized by t-SNE dataset has been saved in the {}.'.format(pngpath))

    # 处理输出文件的类型
    outputfile_file_type = args_c.split('.')[-1]
    if outputfile_file_type == 'csv':
        ...
        filename, ext = os.path.splitext(args_i)
        ...
        df['class'] = temp
        DimensionReduction_filename = os.path.abspath('./Results') + os.sep + args_c
        pandas2arff.pandas2arff(df, filename=r'./Results/{}'.format(args_c), wekaname=filename,
